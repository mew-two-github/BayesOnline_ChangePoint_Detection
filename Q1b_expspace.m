...
for t = 1:N
    ...
    for rt = 1:t % Run length can be 0 till t
        ...
        predictive(rt) = log(tpdf(xt_normalised,2*alpha(t,rt))/std_dev); 
    end
    if t ~= 1
        % Growth probabilities
        % Run length can be 1 till t
        P_joint(t,2:t+1) = (P_joint(t-1,1:t) + predictive(1:t)')+log(1-H);
        % Changepoint probability
        P_joint(t,1) = log(sum(exp(P_joint(t-1,1:t)+predictive(1:t)'+log(H))));
    else
        % P(ro=0) = 1 assumed
        P_joint(2,1) = (predictive(1))+log(1-H);
        P_joint(1,1) = (predictive(1))+log(H);
    end
    % Evidence
    P_evidence = log(sum(exp(P_joint(t,:))));
    ...
    if isnan(sum(P_runlength(t,:))) || isinf(sum(P_runlength(t,:)))
        break
    end
end
plot_rt_probs(real(exp(P_runlength)));